...
public class DbShyeld extends Db{
	private static String userDb="dmeur15"; //dmeur15
	private static String passwordDb="XcU46ay"; //XcU46ay
	...
	public void inscriptionAgent(String nom, String prenom, String identifiant, String mdp){
		try  {
			...
			try(ResultSet rs = tableStatement.get("ia").executeQuery()) {
				while(rs.next()) {
					System.out.println("L'agent a bien �t� ajout� (id dans la bddn :"+rs.getInt("inscription_agent")+")");
				}
	...
	public void suppressionAgent(int id_agent){		
		try {
			...
			try(ResultSet rs= tableStatement.get("sa").executeQuery()) {
				while(rs.next()) {
					System.out.println("L'agent � l'id : "+rs.getInt("supprimeragent")+ " a bien �t� supprim� !");
				}
	...
	public void infoPerteVisibiliteSuperHero(){
		System.out.println("Vous avez choisi d'inspecter la perte de visibilit� des super h�ros");
		try {
	...
	public void listerZonesConflits(){
		System.out.println("Vous avez choisi d'inspecter les zones � conflits");
		try {
	...
	public void classementVictoires(){
		System.out.println("Vous avez choisi de voir le classement des victoires des super-h�ros");
	...
	public void classementReperages(){
		System.out.println("Vous avez choisi de voir le classement des rep�rages des agents");
		try {
